...
import 'package:gymflow/features/workout_templates/presentation/create_workout_template_view_model.dart';
import 'package:provider/provider.dart';
import 'package:gymflow/features/workout_templates/presentation/configure_workout_template_day_page.dart';

class CreateWorkoutTemplatePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => CreateWorkoutTemplateViewModel(
        WorkoutTemplatesService(
          context.read<ApiClient>(),
        ),
      ),
      child: _CreateWorkoutTemplateView(
...
class _CreateWorkoutTemplateView extends StatelessWidget {
  const _CreateWorkoutTemplateView({
  ...
  Widget build(BuildContext context) {
    final viewModel =
    context.watch<CreateWorkoutTemplateViewModel>();
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Crie uma estrutura de treino que poderá ser reutilizada para diferentes alunos.',
                  ...
                  Text(
                    'Organize o modelo em dias e configure os exercícios de cada treino.',
                    style: theme.textTheme.bodyMedium?.copyWith(
                      color: colorScheme.onSurfaceVariant,
                    ),
                  ),
  ...
  Future<void> _editDay(
    ...
    final editedDay = await Navigator.of(context)
        .push<WorkoutTemplateDraftDay>(
      MaterialPageRoute(
        builder: (_) => ConfigureWorkoutTemplateDayPage(
    ...
    final currentExerciseCount =
        viewModel.days[dayIndex].exercises.length;

    for (var index = currentExerciseCount - 1;
    index >= 0;
    index--) {
    ...
    for (final exercise in editedDay.exercises) {
  ...
  Future<void> _addDay(
    ...
    final day = await Navigator.of(context)
        .push<WorkoutTemplateDraftDay>(
      MaterialPageRoute(
        builder: (_) => ConfigureWorkoutTemplateDayPage(
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
            'Adicione o primeiro dia para começar a montar este modelo.',
